...
int BaseEngine::InitialiseObjects()
{
	...
	// Where MAXNUMBER is the number of displayable objects you want
	// CreateObjectArray(MAXNUMBER) ;
...
// mxh03u: Changed to use m_vpDisplayableObjects (vector) instead of m_ppDisplayableObjects (array)
void BaseEngine::CreateObjectArray(int iNumberObjects)
{
	// Destroy any existing object array if it exists
	DestroyOldObjects();
	// Vector code
	// resize vector and initialise each element to a NULL pointer (0)
	m_vpDisplayableObjects.resize(iNumberObjects + 1, NULL);
}


// mxh03u: Added so the vector can be resized by subclasses.
// If the new size is bigger, it is grown and the new elements are initialised to null
// If the new size is smaller, it is shrunk and elements outside the new size are removed
void BaseEngine::ResizeObjectArray(int iNewNumberObjects)
{
	// We have to manually delete objects now outside the range
	for (int i = m_vpDisplayableObjects.size() - 1; i > iNewNumberObjects; i--){
		delete m_vpDisplayableObjects[i];
	}

	m_vpDisplayableObjects.resize(iNewNumberObjects + 1, NULL);
}
...
{
	// Vector code
	m_vpDisplayableObjects[iIndex] = pObject;
}
...
void BaseEngine::DestroyOldObjects()
{
	...
	// Vector code
	if (!m_vpDisplayableObjects.empty()){
		// Before clearing the vector, we need to destroy each element or we'll get a memory leak
		for (std::vector<DisplayableObject*>::const_iterator i = m_vpDisplayableObjects.begin();
			i != m_vpDisplayableObjects.end() && *i != NULL; i++){
			delete *i;
		}

		// Reset its size to 0
		m_vpDisplayableObjects.clear();
	}
}
...
void BaseEngine::UpdateAllObjects( int iCurrentTime )
{
	...
	if (!m_vpDisplayableObjects.empty())
	{
		for (std::vector<DisplayableObject*>::iterator i = m_vpDisplayableObjects.begin();
			i != m_vpDisplayableObjects.end() && *i != NULL; i++){
...
DisplayableObject* BaseEngine::GetDisplayableObject( int iIndex )
{
	...
	if (!m_vpDisplayableObjects.empty())
	{
		return m_vpDisplayableObjects[iIndex];
	}
	else
		return NULL;
}
...
// mxh03u CHANGED: Added parameter to center the text rather than left-align it
...
int BaseEngine::NotifyAllObjectsGetSum( int iSignalNumber )
{
	...
	if (!m_vpDisplayableObjects.empty())
	{
		for (std::vector<DisplayableObject*>::iterator i = m_vpDisplayableObjects.begin();
			i != m_vpDisplayableObjects.end() && *i != NULL; i++){
			iReturn += (*i)->Notify(iSignalNumber);
		}
	}
	return iReturn;
}
...
int BaseEngine::NotifyAllObjectsGetMax( int iSignalNumber )
{
	...
	if (!m_vpDisplayableObjects.empty())
	{
		for (std::vector<DisplayableObject*>::iterator i = m_vpDisplayableObjects.begin();
			i != m_vpDisplayableObjects.end() && *i != NULL; i++){
			int ival = (*i)->Notify(iSignalNumber);
			if (ival > iReturn)
				iReturn = ival;
		}
	}
